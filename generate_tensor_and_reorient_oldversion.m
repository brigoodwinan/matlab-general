...
pathname = [pathname,stupid];
fprintf('\n\n\nOutput files will be saved to the following directory:\n')
disp(pathname)
disp(' ')
disp('The following transform will be applied to the all tensors:')
disp(transformfile)
disp(xfm(1:3,1:3))
...
rdti = rdti(:,[1,2,3,2,4,5,3,5,6]);

% fprintf('\n\n----\nFlipping tensors and appling transformation for head FEM...\n----\n\n\n')
% 
% mfl = makehgtform('xrotate',pi);
% mfl = mfl(1:3,1:3);
% m = xfm(1:3,1:3); % flipping and then rotation for coordinate frame.
% 
% m1 = sparse(m);
% for k = 1:nextpow2(N)
%     m1 = blkdiag(m1,m1);
...
% m1 = m1(1:N*3,1:N*3);
% 
% fprintf('----\nFlipping tensors for tractography...\n----\n\n\n')
% 
% mfl1 = sparse(mfl);
% for k = 1:nextpow2(N)
%     mfl1 = blkdiag(mfl1,mfl1);
% end
% mfl1 = mfl1(1:N*3,1:N*3); % Flipping from Radiological to Neurological
% 
% rdti = reshape(rdti',3,[])';

rdti = double(rdti);

% For performing tractography in SCIRun (tractography must be done on a
% LatVol that has not been transformed etc.).
% dti_tract = mfl1*rdti*mfl';
dti_tract = xfm3d(rdti,mfl);

% For anisotropic head FEM
% dti_headFEM = m1*dti_tract*m';
dti_headFEM = xfm3d(dti_tract,m);

% dti_tract = reshape(dti_tract',9,[])';
% dti_headFEM = reshape(dti_headFEM',9,[])';
